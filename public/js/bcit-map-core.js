...
// Fixed and consolidated public/js/bcit-map-core.js
// - Single navigation engine (graph-based)
// - Safe Mapbox checks before layer/source operations
// - Caching for nodes/graph (initNodeGraph)
// - Graceful fetch error handling
// - Preserves all existing features (favorites, overlays, indoor requests)
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const clearNavigation = () => {
    ...
    try { // remove existing outdoor highlight path
  ...
  async function requestAndOverlayIndoorPath(startBuildingCode, startRoomOrEntrance, goalBuildingCode, goalRoom) {
    ...
    try {
      // Clear any previous overlay before fetching new
      try {
        if (map.getLayer('path-image-layer')) map.setLayoutProperty('path-image-layer', 'visibility', 'none');
      } catch (e) { }
  ...
  const navigateToRoom = (payload) => {
    ...
    navActive = true;
    renderNavPanel(startLngLat === lastUserLocation ? 'Your location' : 'Selected point', toLabel);

    // Trigger hybrid path computation
  ...
  async function computePathFromGraph(startCoord, endNodeList, indoorPayload) {
    ...
    try {
      ...
      let endId = null; best = Infinity; endNodeList.forEach((candidate) => { const coords = nodeMap.get(candidate); if (!coords) return; const d = haversineDistance(coords, startCoord); if (d < best) { best = d; endId = candidate; } });
      ...
      if (path && path.length > 0) {
        ...
        if (indoorPayload) {
          ...
          const startEntranceLabel = NODE_TO_ENTRANCE[endNode] || '';
  ...
  // keep legacy function for debugging but do not call it
  async function loadNodes_DO_NOT_USE() { console.warn('Deprecated: loadNodes_DO_NOT_USE called'); }

  const app = initializeApp(window.firebaseConfig);
  const auth = getAuth(app);

  async function getIdToken() { const user = auth.currentUser; if (!user) throw new Error('Not authenticated. Please log in.'); return await user.getIdToken(true); }
  ...
  async function loadFavoriteMarkers() {
    try {
      ...
      const favNodes = favorites.map(fav => { const node = nodes.find(n => n.id === fav.nodeId); if (!node) return null; let label = fav.label; if (!label && fav.addedAt?._seconds) label = new Date(fav.addedAt._seconds * 1000).toLocaleDateString(); return { node, label }; }).filter(Boolean);
      if (window.favoriteMarkers) window.favoriteMarkers.forEach(m => m.remove()); window.favoriteMarkers = [];
      favNodes.forEach(({ node, label }) => {
        const marker = new mapboxgl.Marker({ color: '#FFD700' }).setLngLat([parseFloat(node.long), parseFloat(node.lat)]).setPopup(new mapboxgl.Popup().setHTML(`⭐ <strong>${label}</strong><br>`)).addTo(map);
  ...
  map.on('load', async () => {
    try {
      ...
      const selSrc = 'building-selected'; if (!map.getSource(selSrc)) { map.addSource(selSrc, { type: 'geojson', data: { type: 'FeatureCollection', features: [] } }); map.addLayer({ id: 'building-selected-line', type: 'line', source: selSrc, paint: { 'line-color': '#f59e0b', 'line-width': 3 } }); }
      if (!map.getSource('nav-route')) { map.addSource('nav-route', { type: 'geojson', data: { type: 'FeatureCollection', features: [] } }); map.addLayer({ id: 'nav-route-line', type: 'line', source: 'nav-route', paint: { 'line-color': '#0ea5e9', 'line-width': 4, 'line-opacity': 0.9 } }); }
      if (SHOW_ALL_LINKS) { loadFavoriteMarkers(); }
      const utils = { getJSON, asLines, geometryBounds, roughCenter, sortFloorsBottomFirst, pdfExists, filterExistingPDFs, buildPopupHTML };
      window.BCITMap = window.BCITMap || {};
      window.BCITMap.map = map; window.BCITMap.utils = utils; window.BCITMap.navigateToRoom = navigateToRoom; window.BCITMap.clearNavigation = clearNavigation; window.BCITMap.geolocateControl = geoControl; window.BCITMap.setCustomStartLocation = setCustomStartLocation; window.BCITMap.clearCustomStart = clearCustomStart; window.BCITMap.setStartFromRoom = setStartFromRoom; window.BCITMap.focusRoom = window.BCITMap.focusRoom || function () { };
      const plugins = window.BCITMapPlugins || [];
      plugins.forEach((fn) => { try { fn(map, utils); } catch (err) { console.error('[BCIT MAP] Plugin error:', err); } });
    } catch (err) { console.error('map.load failed:', err); }
  });
  // Remove indoor path overlay on outside click
  map.on('click', (e) => {
    // If no active navigation, do nothing
    if (!navActive) return;

    // Check if click is on start/end markers
